...
public class PolyService {
	...
	public void ex1() {
		...
		// car의 한개의 객체가 다양한 모습을 띄고있는 것을 다형성이라고 함
		...
		// 2) 다형성을 이용한 객체 배열
		// 객체 배열: 같은 (객체 참조) 자료형 변수를 하나의 묶음으로 다루는 것
				// 같은 객체 => 부모타입으로 바꿈
				// 부모 타입 참조 자료형
		...
		//arr[0] Car 참조 변수
		// arr참조변수는 car 타입 변수이기 때문에 car를 대입해도 문제 안됨
	...
	public void ex3() {
		...
		// "." 연산자가 (Tsela) 형변환 연산자보다 우선순위가 높음
		// 먼저 c1을 Tesla로 형변환 해주고, getBatteryCapacity 호출해야하함.(괄호 사용)
		...
		Tesla t1 = (Tesla)c1;
					// (Tesla)를 빼면 c1 Car타입
		// 자식참조변수로 부모객체 참조? X
		// 부모참조변수로 자식객체는 참조 가능(업캐스팅)
		// C1이 참조하는게 Tesla라는걸 알기때문에 Tesla로 강제형변환 가능.
		// Tesla = Tesla 같은 자료형 연산 가능하니 같은 주소를 복사시켜버림.
	...
	public void ex4() {
		...
		// 부모에서 자식끼리는 연결 되있지만 자식끼리는 관계없음.
		// 따라서 다운캐스팅 예외 발생
		
		
		// 문제점 1.
		// Spark가 Tesla를 참조함. 부모를 공유할 뿐 다른건 겹치지 않음.
		...
		// instanceof와 같이 사용
		if(c1 instanceof Spark) {	// c1의 타입이 Spark가 맞는지 검사
	...
	public void ex5() {
		...
		// String edu.kh.poly.model.vo.Car.getEngin()
		...
		// 실행해보면 wktlr(Spark)의 toString()이 호출되는것을 확인!
	...
	// Tesla 타입의 매개변수만을 받을 수 있음. printCar2() 에 s,c 대입하면 오류
	...
	public Car createCar(int num) {
		...
		case 2: result = new Tesla(); break; // 다형성 업캐스팅이 된 모양
		...
		// 반환형이 Car이지만
		// case가 2, 3이면 Car의 자식 객체 주소가 반환된다.
		
	}
	
	public void method(Car c) {	// 부모타입이 자식 기능을 받아와서 코드길이 감소
								// 업캐스팅으로 자식들을 묶고,다운 캐스팅으로 자식들 기능 사용하게 함
