...
public class AbstractService {
	...
	/*	
		...
		*/
		...
		TypeBirds[] birds = new TypeBirds[] { chiken, eagle, penguin };
		TypePisces[] pisces = new TypePisces[] { shark };
		
		Animal[] animal = new Animal[] { cat,dog,whale,chiken, eagle, penguin, shark};
		// mammalia[0],birds[2]
		...
		for(int i = 0; i < animal.length; i++) {
			...
			// 수유를 한다 == breastfeed() 츌력
			if(animal[i] instanceof Cat) {
				System.out.println("======고양이는요!======");
				((Cat)animal[i]).breastfeed(); // .점이 우선순위가 높기 때문에 형변환 먼저 해준다.
